...
class static_map:
    def __init__(self):
        ...
        self.scene_graph_path = rospy.get_param('~scene_graph_path')
        self.hydra_object_info_topic = rospy.get_param('~hydra_object_info_topic')
        ...
        self.sub = rospy.Subscriber(self.hydra_object_info_topic, ObjectInfo, self.callback)
        signal.signal(signal.SIGINT, self.handler)

    def handler(self,signum,frame):
        if len(self.static_objects)>0:
            with open(self.scene_graph_path+'static_objects.pkl', 'wb') as outp:
                    print("DUMPING")
    ...
    def visualize_scene_graphs(self,objects):
        if len(objects)>0:
            ...
            for x in objects:
                ...
                self.G.add_edge(self.room,obj,label='in') ###
            layout = self.hierarchy_pos(self.G,self.room)
            nx.draw(
